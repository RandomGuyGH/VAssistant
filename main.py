...
# NOVO: Adicione um sprite para Photoshop se você tiver um (opcional)
SPRITE_PHOTOSHOP = "photoshop" # Nome do seu sprite para Photoshop
...
def determine_new_target():
    ...
    if distance > 0:
        ...
        # If distance is zero, it means target is current position, re-determine
        determine_new_target()
    ...
    if moving:
        ...
        if random.random() < 0.005:
            ...
            if random.random() < 0.5: # 50% de chance de ser um fato
                ...
                show_speech_bubble("Pausa para um café!", duration=4)
        ...
        if hit_boundary:
            ...
            show_speech_bubble("Opa! Cheguei no limite.", duration=2)
            determine_new_target() 
            root.after(50, mover_personagem_suave)
            return
